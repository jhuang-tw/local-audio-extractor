...
if "%~1"=="" (
  ...
  echo   extract-fast.bat "C:\path\video.mp4"
  exit /b 64
)
...
set "OUTPUT=%~dpn1.audio.mka"
if not exist "%OUTPUT%" goto output_ready
for /L %%N in (2,1,9999) do (
  if not exist "%~dpn1.audio-%%N.mka" (
    set "OUTPUT=%~dpn1.audio-%%N.mka"
    goto output_ready
  )
)
...
:output_ready
set "PARTIAL=%OUTPUT%.partial-%RANDOM%-%RANDOM%.mka"
echo Input:  %INPUT%
echo Output: %OUTPUT%

ffmpeg -hide_banner -loglevel error -nostdin -y -i "%INPUT%" -map 0:a:0 -vn -c:a copy "%PARTIAL%"
if errorlevel 1 (
