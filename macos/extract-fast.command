...
if [ "$#" -lt 1 ]; then
  echo 'Usage: extract-fast.command "/path/video.mp4"'
  exit 64
fi
...
stem=${name%.*}
base="$dir/$stem"
output="$base.audio.mka"
if [ -e "$output" ]; then
  n=2
  while [ "$n" -le 9999 ]; do
    candidate="$base.audio-$n.mka"
    if [ ! -e "$candidate" ]; then
      output=$candidate
      break
    fi
...
partial="$output.partial.$$.$RANDOM.mka"
cleanup() { rm -f "$partial" >/dev/null 2>&1 || true; }
trap cleanup EXIT HUP INT TERM

echo "Input:  $input"
echo "Output: $output"
if ! ffmpeg -hide_banner -loglevel error -nostdin -y -i "$input" -map 0:a:0 -vn -c:a copy "$partial"; then
